...
import { calculateRankings, createId, loadSession, quizFingerprint, saveSession, validateStart } from "./utils";
  ...
  return <main className="setup-shell">
    ...
    <section className="setup-grid">
      ...
      <div className="panel mode-panel"><div className="panel-heading"><div><span className="step">02</span><h2>Choose the format</h2></div></div>
        ...
        <div className="launch-card"><div><span className="eyebrow">Ready check</span><strong>{errors.length ? `${errors.length} thing${errors.length === 1 ? "" : "s"} to fix` : "Everything looks good"}</strong></div>{errors.length > 0 && <ul>{errors.map((error) => <li key={error}>{error}</li>)}</ul>}<button className="launch" disabled={errors.length > 0} onClick={() => dispatch({ type: "START" })}>Start the quiz <span>→</span></button><button className="text-button" onClick={reset}>Clear saved setup</button></div>
      </div>
    </section>
  </main>;
}

function Scoreboard({ rankings, session, dispatch, questionId, basePoints, onHistory }: { rankings: RankedParticipant[]; session: GameSession; dispatch: React.Dispatch<Parameters<typeof gameReducer>[1]>; questionId: string; basePoints: number; onHistory: (id: string) => void }) {
  const existing = session.questionResults.find((result) => result.questionId === questionId)?.awards ?? [];
  const [selected, setSelected] = useState<Record<string, boolean>>(() => Object.fromEntries(existing.map((a) => [a.participantId, true])));
  const [amounts, setAmounts] = useState<Record<string, string>>(() => Object.fromEntries(existing.map((a) => [a.participantId, String(a.points)])));
  const [placements, setPlacements] = useState(["", "", ""]);
  const [placementPoints, setPlacementPoints] = useState(["3", "2", "1"]);
  ...
  const apply = () => {
    const awards: ScoreAward[] = rankings.filter((p) => selected[p.id]).map((p) => ({ participantId: p.id, points: Number(amounts[p.id] ?? basePoints) })).filter((a) => Number.isFinite(a.points));
    dispatch({ type: "AWARD", questionId, awards });
  };
  const applyPlacements = () => {
    const awards = placements.map((participantId, index) => ({ participantId, points: Number(placementPoints[index]) })).filter((award) => award.participantId && Number.isFinite(award.points));
    dispatch({ type: "AWARD", questionId, awards });
  };
  return <aside className="score-panel"><div className="score-heading"><div><span className="eyebrow">Live standings</span><h2>Scoreboard</h2></div><span className="live-dot">Live</span></div><div className="score-list">{rankings.map((participant) => <div className={`score-row ${participant.rank === 1 ? "leader" : ""} ${existing.some((a) => a.participantId === participant.id) ? "awarded" : ""}`} key={participant.id}><div className="rank">{participant.rank}</div><div className="participant"><button className="participant-name" onClick={() => onHistory(participant.id)}>{participant.name}</button>{participant.members && <small>{participant.members.join(" · ")}</small>}</div><strong className="score">{participant.score}</strong><div className="quick-score"><button aria-label={`Remove one point from ${participant.name}`} onClick={() => adjust(participant.id, -1, "Quick -1")}>−</button><button aria-label={`Add one point to ${participant.name}`} onClick={() => adjust(participant.id, 1, "Quick +1")}>+</button></div><label className="award-toggle"><input type="checkbox" checked={!!selected[participant.id]} onChange={(e) => { setSelected({ ...selected, [participant.id]: e.target.checked }); if (e.target.checked && amounts[participant.id] === undefined) setAmounts({ ...amounts, [participant.id]: String(basePoints) }); }}/><span>Award</span></label><input className="points-input" aria-label={`Points for ${participant.name}`} type="number" step="0.5" value={amounts[participant.id] ?? basePoints} onChange={(e) => setAmounts({ ...amounts, [participant.id]: e.target.value })}/></div>)}</div><button className="apply-awards" onClick={apply}>Apply awards</button><details className="placement-presets"><summary>Placement presets</summary>{["First", "Second", "Third"].map((label, index) => <div className="placement-row" key={label}><span>{label}</span><select aria-label={`${label} place participant`} value={placements[index]} onChange={(e) => { const next = [...placements]; next[index] = e.target.value; setPlacements(next); }}><option value="">Choose…</option>{rankings.map((participant) => <option key={participant.id} value={participant.id}>{participant.name}</option>)}</select><input aria-label={`${label} place points`} type="number" step="0.5" value={placementPoints[index]} onChange={(e) => { const next = [...placementPoints]; next[index] = e.target.value; setPlacementPoints(next); }}/></div>)}<button onClick={applyPlacements}>Apply placements</button></details><p className="score-hint">Reapplying replaces this question’s earlier awards.</p></aside>;
}
...
function QuizScreen({ quiz, session, dispatch, rankings, onEnd, onHistory }: { quiz: QuizFile; session: GameSession; dispatch: React.Dispatch<Parameters<typeof gameReducer>[1]>; rankings: RankedParticipant[]; onEnd: () => void; onHistory: (id: string) => void }) {
  ...
  return <main className="quiz-shell"><header className="quiz-header"><div className="quiz-brand"><span className="brand-mark small">Q</span><div><strong>{quiz.title}</strong><span>{session.status === "paused" ? "Paused" : "Host control"}</span></div></div><div className="progress-block"><div><span>Question {session.currentQuestionIndex + 1} of {quiz.questions.length}</span><span>{Math.round(((session.currentQuestionIndex + 1) / quiz.questions.length) * 100)}%</span></div><div className="progress"><span style={{ width: `${((session.currentQuestionIndex + 1) / quiz.questions.length) * 100}%` }}/></div></div><div className="header-actions"><button onClick={() => document.fullscreenElement ? document.exitFullscreen() : document.documentElement.requestFullscreen()}>Fullscreen</button><button onClick={() => dispatch({ type: session.status === "paused" ? "RESUME" : "PAUSE" })}>{session.status === "paused" ? "Resume" : "Pause"}</button><button className="danger" onClick={onEnd}>End quiz</button></div></header>
    <div className={`quiz-grid ${session.status === "paused" ? "is-paused" : ""}`}><section className="question-stage"><div className="question-meta"><span className="category">{question.category ?? "General"}</span><span>{question.points} {question.points === 1 ? "point" : "points"}</span>{question.choices && <span className="choice-badge">Multiple choice</span>}</div><p className="question-kicker">Question {String(session.currentQuestionIndex + 1).padStart(2, "0")}</p><h1>{question.question}</h1>
      {question.choices && <section className="choice-panel" aria-label="Possible answers"><div className="choice-instruction">{question.answerType === "multiple" ? `Choose ${question.answers.length}` : "Choose one"}</div><div className="choice-grid">{question.choices.map((choice, index) => { const correct = session.answerRevealed && question.answers.some((answer) => answer.trim().toLocaleLowerCase() === choice.trim().toLocaleLowerCase()); return <div className={`choice-option ${correct ? "correct" : ""}`} key={choice}><span>{String.fromCharCode(65 + index)}</span><strong>{choice}</strong>{correct && <em>Correct</em>}</div>; })}</div></section>}
      ...
      <div className="question-actions"><button disabled={session.currentQuestionIndex === 0} onClick={() => dispatch({ type: "GO_TO", index: session.currentQuestionIndex - 1 })}>← Previous</button><button className="reveal" onClick={() => dispatch({ type: "REVEAL", value: !session.answerRevealed })}>{session.answerRevealed ? "Hide answer" : "Show answer"}</button><button onClick={() => isLast ? setNavigator(true) : dispatch({ type: "GO_TO", index: session.currentQuestionIndex + 1 })}>{isLast ? "Review quiz" : "Next →"}</button></div><div className="secondary-actions"><button className="text-button" onClick={() => dispatch({ type: "SKIP", questionId: question.id })}>{result?.skipped ? "Skipped" : "Mark complete without points"}</button><button className="text-button" onClick={() => setNavigator(true)}>Question navigator</button></div></section>
      <Scoreboard key={question.id} rankings={rankings} session={session} dispatch={dispatch} questionId={question.id} basePoints={question.points} onHistory={onHistory}/></div>
    ...
    {navigator && <Dialog title="Question review" onClose={() => setNavigator(false)}><div className="review-summary"><strong>{session.questionResults.filter((r) => r.completed).length} / {quiz.questions.length}</strong><span>questions completed</span></div><div className="navigator-grid">{quiz.questions.map((q, index) => { const r = session.questionResults.find((item) => item.questionId === q.id); const status = r?.skipped ? "skipped" : r?.awards.length ? "scored" : r?.completed ? "complete" : index === session.currentQuestionIndex ? "current" : "unseen"; return <button className={status} key={q.id} onClick={() => { dispatch({ type: "GO_TO", index }); setNavigator(false); }}><strong>{index + 1}</strong><span>{status}</span></button>; })}</div><div className="dialog-actions"><button onClick={() => setNavigator(false)}>Keep playing</button><button className="danger" onClick={onEnd}>Finish quiz</button></div></Dialog>}
  </main>;
}
...
export default function App() {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => { const target = event.target as HTMLElement; if (["INPUT", "TEXTAREA", "SELECT"].includes(target.tagName) || !quiz || !["active", "paused"].includes(session.status)) return; if (event.code === "Space") { event.preventDefault(); dispatch({ type: "REVEAL", value: !session.answerRevealed }); } if (event.key === "ArrowRight" && session.status === "active" && session.currentQuestionIndex < quiz.questions.length - 1) dispatch({ type: "GO_TO", index: session.currentQuestionIndex + 1 }); if (event.key === "ArrowLeft" && session.status === "active" && session.currentQuestionIndex > 0) dispatch({ type: "GO_TO", index: session.currentQuestionIndex - 1 }); if (event.key.toLowerCase() === "p") dispatch({ type: session.status === "paused" ? "RESUME" : "PAUSE" }); if (event.key.toLowerCase() === "f") { if (document.fullscreenElement) void document.exitFullscreen(); else void document.documentElement.requestFullscreen(); } };
    document.addEventListener("keydown", handler); return () => document.removeEventListener("keydown", handler);
  }, [quiz, session.status, session.currentQuestionIndex, session.answerRevealed]);
